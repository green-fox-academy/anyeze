...
app.get('/', (req, res) => {
  let givenAlcohol = req.query.alcohol;
  if (givenAlcohol) {
    const resultCocktails = cocktails.filter(alcohol => {
      return alcohol.contains.includes(givenAlcohol);
    });
    res.render('home', {
        cocktails: resultCocktails, alcohols: alcoholList });
  } else {
    res.render('home', { 
        cocktails: cocktails, alcohols: alcoholList });
  }
});